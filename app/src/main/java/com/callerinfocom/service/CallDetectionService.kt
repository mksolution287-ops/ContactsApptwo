...
import com.callerinfocom.ui.overlay.*
            ...
            override fun onCallStateChanged(state: Int, phoneNumber: String?) {
                when (state) {
                    ...
                    TelephonyManager.CALL_STATE_IDLE -> {
                        when {
                            callInProgress -> {
                                // Answered call (incoming or outgoing) just ended
                            ...
                            wasRinging -> {
                                // Incoming call was missed or rejected before being answered
                                wasRinging = false
                                scope.launch {
                                    delay(1_500)
                                    fetchLastCallAndShowOverlay()
...
//        if (!prefs.getBoolean("callback_screen_enabled", true)) return
//
...
private suspend fun fetchLastCallAndShowOverlay() {
    ...
    // Suppress the app-open ad that would fire when the app comes
    // to the foreground via PostCallOverlayActivity
